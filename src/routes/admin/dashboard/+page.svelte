...
<script>
  ...
  function computeDerived(residents) {
    ...
    for (const r of residents) {
      ...
      // Gender — form saves as r.sex ("Male"/"Female"), fall back to r.gender
  ...
  async function initCharts() {
    await tick(); // wait for Svelte to render canvases (they're behind {#if loading})
    ...
    // Feed in current data immediately after init
    updateRegChart();
    updateGenderChart();
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      const unsubAuth = onAuthStateChanged(auth, async (user) => {
        ...
        // Recent activity feed (last 8 submissions, any status)
        unsubs.push(onSnapshot(
...
<div class="page">
  ...
  <div class="g4">
    <div class="stat-card stat-blue">
      ...
      <p class="stat-sub">{approvalPct}% of all submissions</p>
    </div>
    <div class="stat-card stat-green">
      <p class="stat-label">Senior Citizens</p>
      <p class="stat-val">{seniorCitizens.toLocaleString()}</p>
      <p class="stat-sub">{totalResidents ? Math.round((seniorCitizens/totalResidents)*100) : 0}% of residents</p>
    </div>
    <div class="stat-card stat-amber">
      <p class="stat-label">PWDs</p>
      <p class="stat-val">{pwds.toLocaleString()}</p>
      <p class="stat-sub">{totalResidents ? Math.round((pwds/totalResidents)*100) : 0}% of residents</p>
    </div>
    <div class="stat-card stat-purple">
      <p class="stat-label">Single Parents</p>
      <p class="stat-val">{singleParents.toLocaleString()}</p>
      <p class="stat-sub">{totalResidents ? Math.round((singleParents/totalResidents)*100) : 0}% of residents</p>
    </div>
  </div>
    ...
    <div class="card">
      ...
      {:else}
        ...
        <div class="gender-legend">
          <div class="gleg">
            <span class="gdot" style="background:#2563eb;"></span>
            <span class="glabel">Male</span>
            <span class="gval">{genderMale.toLocaleString()}</span>
          </div>
          <div class="gleg">
            <span class="gdot" style="background:#db2777;"></span>
            <span class="glabel">Female</span>
            <span class="gval">{genderFemale.toLocaleString()}</span>
          </div>
          <div class="gleg">
            <span class="gdot" style="background:#7c3aed;"></span>
            <span class="glabel">Other / N/A</span>
            <span class="gval">{Math.max(0, genderOther).toLocaleString()}</span>
          </div>
        </div>
        ...
        {#if Object.keys(civilStatusMap).length > 0}
          ...
          {#each Object.entries(civilStatusMap) as [key, count] (key)}
            <div class="cs-row">
              ...
              <div class="prog-wrap">
                <div class="prog" style="width:{Math.round((count/maxCivil)*100)}%;background:#2563eb;opacity:.7;"></div>
              </div>
  ...
  <!-- ── Bottom row: special sectors + recent activity ── -->
  <div class="g2-bot">

    <!-- Special sectors detail -->
    <div class="card">
    ...
    <div class="card">
      <div class="card-hd">
        ...
        <button class="link-btn" on:click={() => { window.location.href = '/admin/residents'; }}>
          View all →
        </button>
      </div>
        ...
        {#each recentActivity as r (r.id)}
          <div class="act-row">
            ...
            <div class="act-info">
              ...
              <p class="act-meta">
                <span class="act-status" style="color:{activityDotColor(r.status ?? 'pending')};">
                  {activityLabel(r.status ?? 'pending')}
                </span>
...
<style>
  ...
  .btn-pending {
    ...
    color: #92400e; font-size: 0.78rem; font-weight: 700; cursor: pointer;
  ...
  .error-banner {
    ...
    font-size: 0.8rem; font-weight: 600; padding: 10px 14px; border-radius: 10px;
  }
  ...
  @media (max-width: 1100px) {
    .g3-mid { grid-template-columns: 1fr 1fr; }
    .g3-mid .chart-card { grid-column: span 2; }
  }
  @media (max-width: 800px) {
    .g4     { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    .g3-sub { grid-template-columns: repeat(3, minmax(0, 1fr)); }
    .g3-mid { grid-template-columns: 1fr; }
    .g3-mid .chart-card { grid-column: span 1; }
    .g2-bot { grid-template-columns: 1fr; }
  }
  ...
  .go-btn {
    ...
    transition: background .15s, transform .1s; justify-content: center;
  }
