...
>
  ...
  <div class="relative z-10 w-full max-w-lg flex flex-col items-center gap-7">

    <!-- Brand -->
    <div class="animate-fade-up flex items-center gap-4 text-white">
      <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-2xl shadow-lg shrink-0">📍</div>
      <div>
        <h1 class="font-nunito text-3xl font-black tracking-tight leading-none">
          <span class="font-light opacity-75">Geo</span>Profile
        </h1>
        <p class="text-xs text-white/60 leading-relaxed mt-0.5">
          Barangay Pag-Asa · Resident Profiling System
        </p>
      </div>
    </div>

    <!-- Card -->
    <div class="animate-fade-up-d w-full bg-white rounded-3xl shadow-2xl px-8 py-8">
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        <!-- Full name -->
        <div>
          <label for="name" class="block text-[0.68rem] font-bold tracking-widest uppercase text-slate-400 mb-2">Full Name</label>
          <div class="relative">
            <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
            <input id="name" type="text" bind:value={name} placeholder="Juan dela Cruz" disabled={loading}
        ...
        <!-- Position -->
        <div>
          <label for="position" class="block text-[0.68rem] font-bold tracking-widest uppercase text-slate-400 mb-2">Position</label>
          <div class="relative">
            <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            <select id="position" bind:value={position} disabled={loading}
      ...
      <div class="grid grid-cols-2 gap-4 mb-6">
        <!-- Password -->
        <div>
          <label for="password" class="block text-[0.68rem] font-bold tracking-widest uppercase text-slate-400 mb-2">Password</label>
          <div class="relative">
            <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <input id="password" type={showPass ? 'text' : 'password'} bind:value={password}
              placeholder="Min. 8 chars" autocomplete="new-password" disabled={loading}
              class="w-full pl-10 pr-10 py-3 rounded-xl border-2 border-slate-200 bg-slate-50 text-slate-700 text-sm placeholder-slate-300 outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all" />
        ...
        <!-- Confirm Password -->
        <div>
          <label for="confirm" class="block text-[0.68rem] font-bold tracking-widest uppercase text-slate-400 mb-2">Confirm Password</label>
          <div class="relative">
            <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <input id="confirm" type={showConf ? 'text' : 'password'} bind:value={confirm}
            ...
            <button type="button" on:click={() => showConf = !showConf}
              class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-blue-600 transition-colors p-0.5">
              {#if showConf}
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg>
              {:else}
      ...
      <p class="text-center text-xs text-slate-400 mt-4">
        ...
        <button type="button" on:click={() => { window.location.href = '/'; }}
